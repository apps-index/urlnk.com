...
class Cal
{
    ...
    function dl0()
    {
        ...
        return $this->ol(1, $w, $weeks, $w, $end);
        # $days
    }

    function ol($st = 1, $N = 4, $W = 1, $F = 'F', $end = 31)
    {
        $variable = range(1, $end);
...
print_r(get_defined_vars());die;
        ...
        $pieces[] = $this->h($F, $N);
        $pieces[] = '<article>';
        $pieces[] = $this->sec($st, $N, $W, $N);

        $i = 3;
        foreach ($c as $key => $value) {
            $a = array_shift($value);
            $z = array_pop($value);
            $pieces[] = $this->sec($a, $z, $W, 0);
            $i++;
        }
    ...
    function ol0($st = 1, $ed = 4, $i = 1, $n = 3, $end = 31)
    {
        $start = 6 === $ed ? 1 : $ed + 1;
        $r = range($start, $end);
        // print_r(get_defined_vars());die;
